...
export default class Calculator extends CQRS.Commands.CommandHandler {
    ...
    sum(data: any): Promise<any> {
        ...
        let spMethod: any = Q.async(function* () {
            ...
            yield self.store.saveEvents(aggregate, function (domainEvent: any) {
                ...
                CQRS.ServiceLocator.Process.Instance.publishEvent(new CQRS.Models.Entities.Command('CalculatorEvent', domainEvent.eventName, domainEvent.event));
    ...
    message(data: any): Promise<any> {
        return new Promise((resolve, reject) => {
            CQRS.ServiceLocator.Process.Instance.publishCommand(new CQRS.Models.Entities.Command('MessageCommand', 'message', data));
            resolve();
        });
    }
